...
int countRightLeafNodes(Tree* p1); //Подсчёт количества листьев дерева
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "5 - вывод дерева\n";
		cout << "6 - подсчёт количества листьев дерева\n";
		cout << "7 - подсчет количества букв\n";
		...
		case 6:
			cout << "Количество листьев = " << countRightLeafNodes(Root->Right) << endl;break;
		case 7:  cout << "\nВведите букву: ";
			cin >> letter;
			n = count(Root, letter);
			cout << "Количество слов, начинающихся с буквы " << letter;
			cout << " равно " << n << endl; break;
		case 8:  delAll(Root); break;
		case 9:  exit(0);
...
int countRightLeafNodes(Tree* p1) { //Подсчёт количества листьев дерева
	...
	// Если текущий узел является листом и правым дочерним узлом, возвращаем 1
	if (p1->Left == nullptr && p1->Right == nullptr && p1->key % 2 == 0) {
		return 1;
	}

	// Рекурсивно обрабатываем левое и правое поддеревья и суммируем результаты
	int leftCount = countRightLeafNodes(p1->Left);
	int rightCount = countRightLeafNodes(p1->Right);
